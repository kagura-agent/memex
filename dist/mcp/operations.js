...
import { z } from "zod";
export function registerOperations(server, store, hooks, home, getClientName) {
    // ---- recall ----
    server.registerTool("memex_recall", {
        ...
        const filter = (category || tag || author || since || before)
    ...
    server.registerTool("memex_pull", {
        ...
        const config = await readSyncConfig(home);
        if (!config.remote) {
            return { content: [{ type: "text", text: "Sync not configured. DO NOT set this up yourself. Tell the user to run in their terminal: npx @touchskyer/memex sync --init && npx @touchskyer/memex sync on" }] };
        }
    ...
    server.registerTool("flomo_import_parse", {
        ...
        try {
            ...
            if (fileStat.size > 10 * 1024 * 1024) {
                return { content: [{ type: "text", text: "Error: File too large (max 10MB)." }], isError: true };
            }
        }
        catch {
        ...
        if (memos.length === 0) {
            return { content: [{ type: "text", text: "No memos found. Expected flomo export HTML format." }], isError: true };
        }
        const summary = memos.map((m, i) => `[${i + 1}] ${m.timestamp} | ${m.title} | tags: ${m.tags.join(", ") || "none"}`).join("\n");
