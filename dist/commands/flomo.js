...
function isValidFlomoWebhookUrl(url) {
    try {
...
export async function readFlomoConfig(memexHome) {
    ...
    try {
        ...
        if (url && !isValidFlomoWebhookUrl(url))
            return {};
...
export async function writeFlomoConfig(memexHome, webhookUrl) {
...
export async function flomoConfigCommand(memexHome, opts) {
    if (opts.setWebhook) {
...
function cardToFlomoContent(data, body) {
    const parts = [];
...
async function pushSingleCard(store, slug, webhookUrl, dryRun, fetchFn = globalThis.fetch) {
    let raw;
    try {
    ...
    try {
        ...
        return { slug, status: "error", message: `Network error: ${e.message}` };
    }
}
/**
...
export async function flomoImportCommand(store, filePath, opts) {
    let html;
    try {
    ...
    for (const memo of memos) {
        ...
        const cardContent = stringifyFrontmatter(memo.content, data);
...
export async function flomoPushCommand(store, memexHome, slugOrOpts, opts) {
    const config = await readFlomoConfig(memexHome);
    if (!config.webhookUrl) {
        return {
