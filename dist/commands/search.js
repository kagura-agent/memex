...
import { EmbeddingCache, embedCards, cosineSimilarity, createEmbeddingProvider, } from "../lib/embeddings.js";
import { join } from "node:path";
const DEFAULT_LIMIT = 10;
export async function searchCommand(store, query, options = {}) {
    ...
    if (!query) {
        const items = await Promise.all(allCards.map(async (c) => {
...
function matchesFilter(data, filter) {
    ...
    if (filter.since) {
        ...
        if (!(created >= filter.since || modified >= filter.since))
            return false;
    }
...
// --- Keyword search ---
async function keywordSearch(query, allCards, shouldPrefix, options) {
...
// --- Semantic search ---
async function semanticSearch(query, allCards, shouldPrefix, options) {
    const rawLimit = options.limit ?? DEFAULT_LIMIT;
    const limit = rawLimit < 0 ? DEFAULT_LIMIT : rawLimit;
    if (limit === 0)
        return { output: "", exitCode: 0 };
...
function groupByStore(allCards) {
    const seen = new Set();
    const result = [];
    for (const card of allCards) {
