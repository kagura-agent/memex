...
export async function organizeCommand(store, lastOrganize) {
    const cards = await store.scanAll();
    if (cards.length === 0)
        return { output: "No cards yet.", exitCode: 0 };
    // Build link graph
    ...
    if (recentCards.length > 0) {
        ...
        for (const slug of recentCards) {
            ...
            for (const neighbor of neighbors) {
                ...
                pairSections.push(`### ${slug} ↔ ${neighbor}\n` +
                    `**${slug}** (${info.title}):\n${excerpt1}\n\n` +
        ...
        if (pairSections.length > 0) {
            ...
            sections.push("## Recently Modified Cards + Neighbors (check for contradictions)\n" +
                capped.join("\n\n") +
