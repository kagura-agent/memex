...
export async function readSyncConfig(home) {
    try {
...
export async function writeSyncConfig(home, config) {
    await mkdir(home, { recursive: true });
    await writeFile(join(home, CONFIG_FILE), JSON.stringify(config, null, 2), "utf-8");
}
// ---- Git helpers ----
    ...
    async init(remote) {
        ...
        if (!url) {
            if (!(await ghAvailable())) {
                throw new Error("Provide a repo URL or install gh CLI (https://cli.github.com).");
            }
...
// ---- Auto-sync helper ----
export async function autoSync(home) {
    const config = await readSyncConfig(home);
    if (!config.auto || !config.remote)
        return;
    try {
...
export async function autoFetch(home) {
    const config = await readSyncConfig(home);
    if (!config.remote)
        return; // silent no-op
    try {
